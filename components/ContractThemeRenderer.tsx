    ...
    <>
      ...
      {viewMode === 'Final' ? (
        ...
        <div className="preview-page-container">
          ...
          {/* Page 2+ - Terms and Conditions - Split clauses across pages (3-4 clauses per page) */}
          {(() => {
            const CLAUSES_PER_PAGE = 3;
            const totalPages = Math.ceil(sortedClauses.length / CLAUSES_PER_PAGE);
            const pages = [];
            
            for (let pageIndex = 0; pageIndex < totalPages; pageIndex++) {
              const startIdx = pageIndex * CLAUSES_PER_PAGE;
              const endIdx = Math.min(startIdx + CLAUSES_PER_PAGE, sortedClauses.length);
              const pageClauses = sortedClauses.slice(startIdx, endIdx);
              const currentPageNum = pageIndex + 2;
              
              pages.push(
                <div key={`clause-page-${pageIndex}`} className="preview-page">
                  <div className="preview-page-header">
                    <span className="font-bold">{profile.companyName}</span>
                    <span>{doc.contractType || 'Contract'}</span>
